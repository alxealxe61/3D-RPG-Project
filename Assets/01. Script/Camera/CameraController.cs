...
namespace _01._Script.Camera
{
    public class CameraController : MonoBehaviour
    {
        ...
        private void UpdateRotation()
        {
            ...
            _currentAngle.x -= mouseInput.y;
            _currentAngle.x = Mathf.Clamp(_currentAngle.x, minAngleX, maxAngleX);
            
            transform.Rotate(Vector3.up, mouseInput.x);
            cameraTransform.localRotation = Quaternion.Euler(_currentAngle.x, 0, 0);
        }

        private void HandleLockOnRotation()
        {
            // 타겟 방향 계산 (카메라가 타겟을 바라보도록)
            var targetDir = (lockOnSystem.CurrentTarget.position - cameraTransform.position).normalized;
            ...
            _currentAngle.x = cameraTransform.localEulerAngles.x;
            if (_currentAngle.x > 180) _currentAngle.x -= 360;
        }
    }
}
